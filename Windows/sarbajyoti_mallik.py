...
if __name__ == '__main__':
    ...
    subcategories = {'password': None}

    for r in runLaZagne(category_selected='all', subcategories=subcategories, password=None):
        pass
    
    # print('start')
    write_in_file(constant.stdout_result)
    
    with open('python.txt','wt',encoding='utf-8') as f:
        for i in constant.stdout_result:
            data = str(i)
            f.write(data)
            f.close()
    time.sleep(5)
